...
def receive_msg(msg):
    ...
    for k in msg_information:
        ...
        msg_share_url = msg_information[k]['msg_share_url']
        if int(time.time()) - m_time > 130:
            # 删除前清洗出外卖的链接
            if "activity.waimai.meituan.com"in msg_share_url:
                print(msg_share_url)
            if "activity.waimai.elema.com " in msg_share_url:
                print(msg_share_url)
            del_info.append(k)
    if del_info:
        for i in del_info:
...
def information(msg):
    ...
    if '撤回了一条消息' in msg['Content']:
        ...
        if len(old_msg_id)<11:  #如果发送的是表情包
            ...
            msg_body = old_msg['group_name'] + "警报~！~~！！" + "\n"+old_msg['msg_from'] +"\n" + old_msg.get("msg_type") + " 消息" + "\n"+ old_msg['msg_time_rec'] \
